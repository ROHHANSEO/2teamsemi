...
<body>
	...
	<div class="wtotal">
		...
		<div class= "selectpart"><!--  최신 카테고리  -->
			<div class="selectparta"><p> 저가순 상품 </p></div>
		...
		<%}else{ %>
		<!-- 맨 처음으로 (<<) -->
		<button onclick="location.href='<%=request.getContextPath()%>/lowerAuction.do?currentPage=1'"> &lt;&lt; </button>
		<%} %>
		...
		<button disabled> &lt; </button>
		<%}else{ %>
		<button onclick="location.href='<%= request.getContextPath() %>/lowerAuction.do?currentPage=<%= currentPage-1 %>'"> &lt; </button>
		<%} %>
		...
		<%for(int p=startPage; p<=endPage; p++){ %>
			...
			<%}else{ %>
			<button onclick="location.href='<%=request.getContextPath() %>/lowerAuction.do?currentPage=<%= p %>'"> <%= p %> </button>
			<%} %>
		...
		<button disabled> &gt; </button>
		<%}else { %>
		<button onclick="location.href='<%=request.getContextPath()%>/lowerAuction.do?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		<%} %>
	</div> 
