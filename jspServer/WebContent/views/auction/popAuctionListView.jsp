...
<body>
	...
	<div class="wtotal">
		...
		<%}else{ %>
		<!-- 맨 처음으로 (<<) -->
		<button onclick="location.href='<%=request.getContextPath()%>/popAutcion.do?currentPage=1'"> &lt;&lt; </button>
		<%} %>
		...
		<button disabled> &lt; </button>
		<%}else{ %>
		<button onclick="location.href='<%= request.getContextPath() %>/popAutcion.do?currentPage=<%= currentPage-1 %>'"> &lt; </button>
		<%} %>
		...
		<%for(int p=startPage; p<=endPage; p++){ %>
			...
			<%}else{ %>
			<button onclick="location.href='<%=request.getContextPath() %>/popAutcion.do?currentPage=<%= p %>'"> <%= p %> </button>
			<%} %>
		...
		<button disabled> &gt; </button>
		<%}else { %>
		<button onclick="location.href='<%=request.getContextPath()%>/popAutcion.do?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		<%} %>
	</div> 
