...
<%
	...
	switch(sc.getCategory()){
		case "운영정책" : selected = "selected";break;
		case "계정/인증" : selected = "selected";break;
		case "구매/판매" : selected = "selected";break;
		case "거래 품목" : selected = "selected";break;
		case "거래 매너" : selected = "selected";break;
		case "이벤트" : selected = "selected";break;
		case "이용 제재" :selected = "selected";break;
		case "커뮤니티" : selected = "selected"; break;
		case "경매" : selected = "selected";break;
		case "채팅" : selected = "selected";break;
		case "기타" : selected = "selected"; break;
	}
%>
<!DOCTYPE html>
<html>
...
<body>
	...
	<form name="updateForm" class="enrollServiceCenterForm" action="<%=request.getContextPath()%>/updateServiceCenter.do" method="post">
	<input type="hidden" name="scno" value="<%=sc.getServiceNo() %>">
	<div class="enrollTitlesection"><!-- 제목 div -->
		<span class="titleNamepart"> 제목<span class="dotpoint">*</span></span>
		<input type="text" name="title" class="counttitlename" value="<%=sc.getServiceTitle() %>" placeholder="제목을 입력해주세요" maxlength="40" required >
	...
	<div class="enrollTitlesection"><!-- 카테고리 선택 div -->
		...
		<div class="categorytogglepart">
			<select class="categoryList"  name="category" required>
				...
				<option class="optionlist" value=2 <%=selected %>>계정/인증</option>
				<option class="optionlist" value=3 <%=selected %>>구매/판매</option>
				<option class="optionlist" value=4 <%=selected %>>거래 품목</option>
				<option class="optionlist" value=5 <%=selected %>>거래 매너</option>
				<option class="optionlist" value=6 <%=selected %>>이벤트</option>
				<option class="optionlist" value=7 <%=selected %>>이용 제재</option>
				<option class="optionlist" value=8 <%=selected %>>커뮤니티</option>
				<option class="optionlist" value=9 <%=selected %>>경매</option>
				<option class="optionlist" value=10 <%=selected %>>채팅</option>
				<option class="optionlist" value=11 <%=selected %>>기타</option>
			</select>
		</div>
	</div>
	<div class="enrollcontentsection"><!-- 내용 작성 div -->
		...
		<div class="contentpart">
			<textarea name ="content" class="contentWritePart" cols="70" rows="15" style="resize:none;" required><%=sc.getServiceContent() %></textarea>
		</div>
		<span class="countContentlength">0 / 2000</span>
	</div>
	
	<div class="deleteorsubmit" align="right"><!-- 삭제, 등록버튼 -->
		<button type="submit" class="commonsubmit gotoSubmitbutton" > 수정하기 </button>
	...
	<script type ="text/javascript">
		...
		function askcancel(){
			var cancel = confirm("글 수정을 취소하시겠습니까?");
			if(cancel){
